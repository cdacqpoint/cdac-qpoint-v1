...
class PostStore extends EventEmitter {
     ...
     * @memberof PostStore
     */
    async createPost({ title, description, category, tags, name, email, notify}) {
        try {
            const categories = category.map(cat => {
                return cat.value;
            })
            const response = await PostsAPI.createQuestion({ title, desc: description, categories, courseTag: tags, name, email, notify })
